...
class ChatbotModel(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.fc1=nn.Linear(input_dim, 128) #128 neurons coonected 64 neurons
...
class ChatbotAssistant:
    def __init__(self, intents_path, function_mappings=None):
        ...
        self.documents=[] #intents.json is the json file while input of the user response is all stored
        self.intents=[]
    ...
    @staticmethod
    def tokenize_and_lemmatize(text):# lemmetizer is used when when we need to make runs as run past to present
...
def get_stocks():
    stocks=['APPL','META','NVDIA','GS','MSFT']
...
if __name__ =='__main__':
    ...
    assisstant=ChatbotAssistant('intents.json', function_mappings={'stocks':get_stocks})
    assisstant.parse_intents()
    assisstant.load_model('chatbot_model.pth','dimensions.json')

    while True:
        message=input('Enter your message:')
        if message=='/quit':
            break
        print(assisstant.process_message(message))